...
class NeRFModulateT(nn.Module):
    def __init__(self, args):
        ...
        self.embedtime_fn, self.input_ch_times = get_embedder(args.time_multires, "latent",
    ...
    def raw2outputs_old(self, raw, z_vals, rays_d, raw_noise_std=0, pytest=False):
    ...
    def render_rays(self,
        ...
        perturb = self.args.perturb if self.training else 0
        raw_noise_std = self.args.raw_noise_std if self.training else 0
    ...
    def forward(self, H, W, t=None, chunk=1024 * 32, intrinsics=None, rays=None, poses=None):
        ...
        if self.training:
            ...
            rgb, depth, acc, extras = self.render(H, W, rays, t, chunk=chunk)
        ...
        #  evaluation
            ...
            if t is None:
                rgbs, depths = self.render_path_vary_t(H, W, chunk, poses, intrinsics)
            else:
                rgbs, depths = self.render_path_fix_t(H, W, t, chunk, poses, intrinsics)
            return self.tonemapping(rgbs), depths

    def render(self, H, W, rays=None, t=None,
               ndc=False, chunk=1024,
               c2w_staticcam=None):  # the render function
        """Render rays
            Args:
        ...
        with torch.no_grad():
            box = torch.tensor(self.roibox).type_as(rays_o)
        ...
        for i in range(0, rays.shape[0], chunk):
            ret = self.render_rays(rays[i:i + chunk], rays_info[i:i + chunk])
            for k in ret:
                if k not in all_ret:
                    all_ret[k] = []
    ...
    def render_path_fix_t(self, H, W, t, chunk, render_poses, intrinsics):
        rgbs = []
        depths = []
    ...
    def render_path_vary_t(self, H, W, chunk, render_poses, intrinsics):
        rgbs = []
        depths = []
...
class NeUVFModulateT(nn.Module):
    def __init__(self, args):
        ...
        elif self.explicit_warp_type == 'proj':  # actuall affine
    ...
    def render(self, H, W, rays=None, t=None,
               ndc=False, chunk=1024, c2w_staticcam=None):  # the render function
        """Render rays
            Args:
              H: int. Height of image in pixels.
              W: int. Width of image in pixels.
